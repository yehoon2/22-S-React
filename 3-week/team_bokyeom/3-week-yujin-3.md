...
React component는 Props(속성들)을 입력받아 React element를 생성

Prop는 react component의 속성

컴포넌트에 전달할 다양한 정보를 담고 있는 자바스크립트 객체

react는 컴포넌트안에 또다른 component를 쓸 수 있다

복잡한 화면을 여러 개의 component로 나눠서 구현 가능

Component 추출

재사용성 증가 (해당 컴포넌트 기능 목적 구체화되고 해당 Props 단순해지기 때문)

개발속도 향상

Function Component

모든 리액트 함수는 Pure 함수같은 역할을 해야한다

즉 react component를 일종의 함수라고 생각한다

Class Component 
자바스크립트 Es6의 클래스라는 것을 이용해 만들어진 Component
...
[이전 섹션](https://ko.reactjs.org/docs/rendering-elements.html#updating-the-rendered-element)에서 다뤄본 째깍거리는 시계 예시를 다시 살펴보겠습니다. [엘리먼트 렌더링](https://ko.reactjs.org/docs/rendering-elements.html#rendering-an-element-into-the-dom)에서는 UI를 업데이트하는 한 가지 방법만 배웠으며, 렌더링 된 출력값을 변경하기 위해 `ReactDOM.render()`를 호출했습니다.
...
1. 초기 `this.state`를 지정하는 [class constructor](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Classes#Constructor_(%EC%83%9D%EC%84%B1%EC%9E%90))를 추가합니다.
...
= 리액트 Component의 상태 (변경가능한 데이터)

사용자가 정의하는데 렌더링이나 데이터 흐름에 사용되는 값만 state에 포함시켜야 한다

state는 자바스크립트 객체이다

state는 직접 수정할 수 없다 ( 엄밀히 말하면 수정이 가능하긴 하나 그러면 안된다)

classcomponent에서 state를 변경하고자 할 때에는 꼭 setState 함수를 사용해야 한다

# Lifecycle

리액트 component가 생성되는 시점과 사라지는 시점이 정해져 있다

 초록색 box : 생명주기에 따라 호출되는 classComponent의 함수들 ( Lifecycle 메소드라고 부름)

 출생 : 생성자 (constructor) 생성

 인생 : setState - 상태정의 , forceUpdate - 강제 업데이트

 사망 : Unmount 되기 바로 직전에 함수 작동
